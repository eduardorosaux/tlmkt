...
class ContactsRepository
{
    ...
    public function all($request)
    {
        ...
        return Contact::latest()->withPermission()->where('type', $request->type)->paginate(setting('pagination'));
    }

    public function blockContacts($request)
    {
        return Contact::latest()->withPermission()->where('type', $request->type)->where('is_blacklist', 1);
    }

    public function getChatContactList($data = []): LengthAwarePaginator
    {
        return $this->model->with(['lastMessage'])->withPermission()->when(arrayCheck('q', $data), function ($q) use ($data) {
        ...
        })->where('has_conversation', 1)->where('status', 1)->orderBy('last_conversation_at', 'DESC')->paginate(12);
    }
    ...
    {
        return Contact::with('contactList.list', 'segmentList.segment')->find($id);
    }
    ...
    {
        return Contact::where('status', 1)->when(arrayCheck('client_id', $data), function ($query) use ($data) {
