...
class WebsiteFaqRepository
{
    public function all()
    {
        ...
        // Set the limit based on the active theme
        if ($theme == 'martex') {
            $limit = 6;
        } elseif ($theme == 'darkbot') {
            $limit = 8;
        } else {
            $limit = 4;
        }
    
        // Build and execute the query with the dynamic 'take' value
        return Faq::active()->take($limit)->latest()->get();
    }
