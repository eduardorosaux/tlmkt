    ...
    private function saveIncommingMessage($contact, $content, $client, $is_contact_msg, $is_campaign_msg, $type, $message_id)
    {
        ...
        try {
            ...
            if ($type == 'image') {
                ...
                $response               = $content['messages'][0]['button']['text'];
                $message->buttons = $response;
                $notified_message      = $response;
            }
            ...
            // ]);
            // $is_read                       = $whatsapp_cloud_api->markMessageAsRead($content['messages'][0]['id']);
            $message->status          = MessageStatusEnum::DELIVERED;
            $message->update();
            ...
            Log::info('Save WhatsApp Incomming', [$e->getMessage()]);
            return false;
        }
    }
}
