<template>
	<div class="sp-right-sidebar">
				...
				<nav class="chat-tab-two">
					<div class="nav nav-tabs border-0" id="nav-author-info-tab" role="tablist">
						<button
							...
							id="nav-authorinfo-tab"
							data-bs-toggle="tab"
							data-bs-target="#nav-authorinfo"
							type="button"
							role="tab"
							aria-controls="nav-authorinfo"
							aria-selected="true"
						>
							{{ getMixinValue.lang.details }}
						</button>
						<button
							class="nav-link"
							id="nav-sharelist-tab"
							data-bs-toggle="tab"
							data-bs-target="#nav-sharelist"
							@click="getSharedMedias"
							type="button"
							role="tab"
							aria-controls="nav-sharelist"
							aria-selected="false"
						>
							{{ getMixinValue.lang.shared_files }}
						</button>
					</div>
				</nav>
				<div class="tab-content" id="nav-chat-tabContent">
					<div class="tab-pane fade active show" id="nav-authorinfo" role="tabpanel" aria-labelledby="nav-authorinfo-tab">
						<div class="sp-details-area">
							<ul class="details-info-list">
								<li>
									<i class="las la-comments"></i>
									<p>
										{{ getMixinValue.lang.conversation_id }} <strong>{{ data.contact.conversation_id }}</strong>
									</p>
								</li>
								<li>
									...
									<p>
										{{ getMixinValue.lang.source }} <strong class="text-capitalize">{{ data.contact.source }}</strong>
									</p>
								</li>
								<li>
									<i class="las la-briefcase"></i>
									<p>
										{{ getMixinValue.lang.id }} <strong>{{ data.contact.id }}</strong>
									</p>
								</li>
								<li>
								...
								<li>
									...
									<p>
										{{ getMixinValue.lang.creation_time }} <strong>{{ data.contact.created_at }}</strong>
									</p>
								</li>
								<li>
									<i class="las la-hourglass-half"></i>
									<p>
										{{ getMixinValue.lang.last_activity }}<strong>{{ data.contact.last_conversation_at }}</strong>
									</p>
								</li>
								<li v-if="data.contact.source == 'whatsapp'">
									<i class="las la-phone-alt"></i>
									<p>
										{{ getMixinValue.lang.phone }} <strong>{{ data.contact.phone }}</strong>
									</p>
								</li>
							</ul>
						</div>
										...
										<div v-if="note.show">
											<div class="position-relative">
												...
												<button
													...
													type="button"
													class="btn"
													style="font-size: 15px; position: absolute; top: -12px; right: 0"
												>
									...
									<div class="right-part">
										<div class="dropdown">
											...
											<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
												<li @click="hideFile(index, 'files')">
													<a class="dropdown-item" href="javascript:void(0)">{{ getMixinValue.lang.hide }}</a>
												</li>
												<li @click="deleteFiles(file.id, index, 'files')">
													<a class="dropdown-item" href="javascript:void(0)">{{ getMixinValue.lang.delete }}</a>
												</li>
											</ul>
										</div>
									</div>
				...
				<template #content>
					<div class="modal-body">
						...
						<div>
							...
							<textarea :placeholder="getMixinValue.lang.add_note" rows="5" v-model="data.note.details"></textarea>
						</div>
					</div>
				</template>
				<template #footer>
		...
		<!--    tag modal-->
		<Transition>
			<Modal class="sp-modal" :isOpen="isTagModalOpened" @modal-close="closeTagModal" name="tag-modal">
				<template #header class="modal-title">
					<div class="row w-100">
						<div class="col-lg-6">
						...
						<div class="col-lg-6 text-end">
							<button @click="closeTagModal" type="button" class="btn" style="font-size: 15px"><i class="las la-times"></i></button>
						</div>
					</div>
				</template>
				<template #content>
		...
		<!--    Setting modal-->
		<Transition>
			<Modal class="sp-modal" :isOpen="isSettingModalOpened" @modal-close="closeSettingModal" name="tag-modal">
				<template #header class="modal-title">
					<div class="row w-100">
						<div class="col-lg-6">
						...
						<div class="col-lg-6 text-end">
							<button @click="closeSettingModal" type="button" class="btn" style="font-size: 15px"><i class="las la-times"></i></button>
						</div>
					</div>
				</template>
				<template #content>
