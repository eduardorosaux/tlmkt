              ...
              <div class="login-form bg-white rounded-20">
                ...
                <form  class="ajax_form" method="POST" action="{{ route('postlogin') }}">
                  ...
                  <div class="mb-30">
                    ...
                    <input type="text" class="form-control rounded-2" id="email" value="{{ old('email') }}" name="email"  placeholder="{{ __('email') }}" required autofocus>
                    <x-input-error :messages="$errors->get('email')" class="mt-2 nk-block-des text-danger" />
                  </div>
                  <div class="mb-30">
                    <label for="password" class="form-label">{{__('password')}} *</label>
                    <input type="password" class="form-control rounded-2" id="password" placeholder="{{ __('password') }}" name="password" required autofocus>
                  ...
                  <span class="text-center d-block">{{__("doesn't_have_an_account_yet?")}} <a href="{{ route('register') }}" class="sg-text-primary">{{__('sign_up')}}</a>?</span>
                  @if (Route::has('password.request'))
                    <span class="text-center d-block mb-20">{{__('forgot_your')}} <a href="{{ route('password.request') }}" class="sg-text-primary">{{__('password')}}</a>?</span>
                  @endif
                  @if(isDemoMode())
                    <div class="login-as">
                      <h6>{{ __('login_as') }}</h6>
                      <ul class="login-BTN d-flex gap-3">
                        ...
                        <li>
                          <a href="javascript:void(0)" class="input_filler template-btn bordered-btn-secondary" data-type="client">{{ __('client') }}</a>
                        </li>
                      </ul>
                    </div>
                  @endif
