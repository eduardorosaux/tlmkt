		...
		<div class="container-fluid">
			<form 
				  ...
				  @if (setting('is_stripe_activated')) 
				  	action="{{ route('client.stripe.redirect', ['trx_id' => $trx_id, 'payment_type' => 'stripe', 'package_id' => $package->id]) }}"
			      @endif
			      @if (setting('is_paypal_activated')) 
				 	action="{{ route('client.paypal.redirect', ['trx_id' => $trx_id, 'payment_type' => 'paypal', 'package_id' => $package->id]) }}"
			      @endif
			      @if (setting('is_paddle_activated')) 
				  	action="{{ route('client.paddle.redirect', ['trx_id' => $trx_id, 'payment_type' => 'paddle', 'package_id' => $package->id]) }}"
			      @endif
											...
											<div class="col-md-4 col-xl-4">
												<div class="mb-3">
													...
													<input type="text"
													       ...
													       required value="{{ Auth::user()->client->name }}">
					...
					<div class="col-lg-3">
						...
						<div class="mt-2 text-center mt-4">
							@if ($package->is_free==1)
								<button type="submit"
									        class="w-100 btn sg-btn-primary d-block">{{ __('proceed') }}</button>
							@else
							<a href="#"
